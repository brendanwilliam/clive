...
Both apps use the same CloudKit container's private database. A custom zone stores opaque per-device records containing only application-encrypted, five-minute endpoint advertisements, WAN gate tokens, replay metadata, or reachability requests. CloudKit notifications are coalescible hints to fetch current records. These records are short-lived discovery state, not a session store.
...
Disabling cellular access or revoking a phone invalidates local gate tokens before CloudKit deletion. Cached records cannot open a new WAN shell. A network-path loss only detaches the transport: the Mac retains the PTY for the 90-minute reattachment grace period while the foreground iPhone retries the same opaque session over LAN, private VPN, public IPv6, then a manual public endpoint. Cellular routes are excluded whenever the iPhone setting is disabled. Reattachment replays bounded output and reports truncation; it never treats a replacement shell as a successful resume.
