...
public enum PairingLink {
    public static let appScheme = "clive"
    public static let appHost = "pair"
    ...
    public static func route(_ url: URL, supportsCurrentVersion: Bool = true, supportsPlatform: Bool = true, now: Date = .now) -> PairingLinkRoute {
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false),
              ((components.scheme?.lowercased() == "https" && components.host?.lowercased() == domain && components.path == path) ||
               (components.scheme?.lowercased() == appScheme && components.host?.lowercased() == appHost && components.path.isEmpty)),
