...
final class TLSIdentityStore {
    ...
    init(baseURL: URL? = nil, fileManager: FileManager = .default) {
        let base = baseURL ?? fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0].appending(path: "clive")
    ...
    private func generateIdentity(password: String) throws {
        ...
        try runOpenSSL(["req", "-x509", "-new", "-key", keyURL.path, "-out", certificateURL.path, "-days", "3650", "-subj", "/CN=clive"])
        try runOpenSSL(["pkcs12", "-export", "-name", "clive", "-out", p12URL.path, "-inkey", keyURL.path, "-in", certificateURL.path, "-passout", "pass:\(password)"])
