...
xcodebuild -project "${ROOT_DIR}/Apps/CliveMac/CliveMac.xcodeproj" -scheme CliveMac -configuration Release -derivedDataPath "${MAC_DERIVED_DIR}" CODE_SIGNING_ALLOWED=NO ARCHS=arm64 build
...
pkgbuild --root "${STAGE_DIR}" --identifier com.clive.cli --version "${VERSION:-0.1.0}" --install-location / "${UNSIGNED_PKG}"
